...
function midiToFrequency(midiNote) {
	return Math.round(440 * 2 ** ((midiNote - 69) / 12) * 100) / 100;
}
...
async function parseMidiFile(filename) {
	...
	midi.tracks.forEach((track, trackIndex) => {
		console.log(
			`   Track ${trackIndex + 1}: ${track.name || 'Unnamed'} (${track.notes.length} notes)`
		);
	...
	backgroundNotes.slice(0, 10).forEach((note, i) => {
		console.log(
			`   ${i + 1}. ${note.name.padEnd(4)} @ ${note.time.toFixed(2)}s - ${note.frequency}Hz (vel: ${note.velocity})`
		);
	});
...
async function parseAllMidiFiles() {
	...
	results.forEach((result, index) => {
		if (result.error) {
			...
			console.log(
				`      Notes: ${result.stats.totalNotes} total (${result.stats.backgroundNotes} bass, ${result.stats.melodyNotes} melody)`
			);
